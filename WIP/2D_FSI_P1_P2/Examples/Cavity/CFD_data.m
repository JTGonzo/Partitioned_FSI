...
%data.force{2} = @(x, y, t, param)(-10^3 + 0.*x.*y);

% Dirichlet
data.bcDir_t  = @(t)( (1-cos(0.4*pi*t)).* (t>=0.0));

data.bcDir{1} = @(x, y, t, param)(data.bcDir_t(t)*((y-0.875)./0.125).*(x==0).*(y>=0.875) + data.bcDir_t(t)*(y==1.0) - data.bcDir_t(t)*(x==0.0).*(y==1.0) + 0.*x.*y); 
...
data.flag_ring{2}         =  [1]; 
data.flag_ALE_fixed{2}    =  [2 3 5];
data.flag_pressure{2}  =  [3];
%data.flag_ring{2}  =  [];

% Model parameters
data.dynamic_viscosity  =   .01;
data.density            =   1;
...
data.Output.DragLift.factor          = 2/(1*0.1);
