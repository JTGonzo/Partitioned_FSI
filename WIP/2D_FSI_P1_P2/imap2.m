...
for k = 1 : dim
    ...
    %% Create map assuming node co-location between domains at interface
    ...
    x_SF{k} = zeros(length(interfaceS_dofs{k}),length(interfaceF_dofs{k}));
    y_SF{k} = zeros(length(interfaceS_dofs{k}),length(interfaceF_dofs{k}));
    ...
    for i = 1 : length(interfaceS_dofs{k})
        ...
        for j = 1 : length(interfaceF_dofs{k})
            ...
            x_SF{k}(i,j) = iS_coord(1) - iF_coord(1);
            y_SF{k}(i,j) = iS_coord(2) - iF_coord(2);
            l_SF{k}(i,j) = sqrt(x_SF{k}(i,j)^2 + y_SF{k}(i,j)^2);
            id_SF{k}(i,j) = j;
        end
    end
    
    for i = 1 : length(interfaceS_dofs{k})
        vecs = [x_SF{k}(i,:)', y_SF{k}(i,:)', l_SF{k}(i,:)', id_SF{k}(i,:)'];
        vec_or = sortrows(vecs,3);
        x_SF{k}(i,:) = vec_or(:,1)';
        y_SF{k}(i,:) = vec_or(:,2)';
        l_SF{k}(i,:) = vec_or(:,3)';
        id_SF{k}(i,:) = vec_or(:,4)';
    ...
    x_FS{k} = zeros(length(interfaceF_dofs{k}),length(interfaceS_dofs{k}));
    y_FS{k} = zeros(length(interfaceF_dofs{k}),length(interfaceS_dofs{k}));
    ...
    for i = 1 : length(interfaceF_dofs{k})
        ...
        for j = 1 : length(interfaceS_dofs{k})
            ...
            x_FS{k}(i,j) = iF_coord(1) - iS_coord(1);
            y_FS{k}(i,j) = iF_coord(2) - iS_coord(2);
            l_FS{k}(i,j) = sqrt(x_FS{k}(i,j)^2 + y_FS{k}(i,j)^2);
            id_FS{k}(i,j) = j;
        end
    end
    
    for i = 1 : length(interfaceF_dofs{k})
        vecs = [x_FS{k}(i,:)', y_FS{k}(i,:)', l_FS{k}(i,:)', id_FS{k}(i,:)'];
        vec_or = sortrows(vecs,3);
        x_FS{k}(i,:) = vec_or(:,1)';
        y_FS{k}(i,:) = vec_or(:,2)';
        l_FS{k}(i,:) = vec_or(:,3)';
