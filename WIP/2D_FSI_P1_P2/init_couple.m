...
Jhat = zeros(length(uS(MESH.Solid.Gamma_global)),length(uS(MESH.Solid.Gamma_global)));  % initial approx. interface Jacobian
...
%         flag_t = 6;    
    case 'broydensecond'
        Acc_flag = 5;
        Couple.reuse = 0;                   
        model = Broyden(Couple.reuse, Jhat);
    case 'genbroyden'
        Acc_flag = 6;
        Couple.reuse = 0;
        model = GB(Couple.small, Couple.reuse, Jhat, Jhat);
...
%     case 'hddf'
%         Acc_flag = 10;
%         model = HDDF(Couple.small,Couple.reuse);
    otherwise
